...
namespace ENTICourse.IK
{
    ...
    public class InverseKinematics : MonoBehaviour
    {
        ...
        public float DistanceFromTarget(Vector3 target, float[] Solution)
        {
            ...
            Vector3 point = ForwardKinematics(Solution);
            float angleError = Mathf.Abs(Solution[1] - Solution[2]) + Mathf.Abs(Solution[1] - Solution[3]) + Mathf.Abs(Solution[2] - Solution[3]);
            return Vector3.Distance(point, target) + angleError / 100;
        }

        public float DistanceMinimize(Vector3 target, float[] Solution)
        {
            Vector3 point = ForwardKinematics(Solution);
            float angleError = Mathf.Abs(Solution[1] - Solution[2]) + Mathf.Abs(Solution[1] - Solution[3]) + Mathf.Abs(Solution[2] - Solution[3]);
            return Vector3.Distance(point, target) + angleError;
        }
